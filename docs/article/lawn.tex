...
\advcomplexity{1}{1}{1}{1}{1}{1}{n}{1}{the Hashed Timing Wheel}
...
As this algorithm iterates over all active TTLs (TTLs that are used by timers which did not expire yet) it's works case runtime is lacking even in comparison to more primitive implementations of timer storage. That said, with the added assumption that the TTL set size is constant, or at worst asymptotically smaller then the number of timers\footnote{This assumptions is valid since this data structure is intended for use in service of a single system. Having only one system set timers entails having a limited and expected range of different TTLs, in many cases millions of timers and tens of TTLs}, we can regard this operation as constant time. Moreover, using an active TTLs and known timers list (containing pointers to the allocated memory of the trigger) and a doubly linked list implementation for the queues, it is possible to get runtime complexity to be:

\advcomplexity{1}{1}{n}{1}{1}{1}{1}{1}{the Lawn}
