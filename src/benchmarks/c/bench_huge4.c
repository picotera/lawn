/* General single-cell runner for the FULL 4-axis sweep (n, ttl_span,
 * distinct_ttls, workload) at a huge fixed baseline instead of the small
 * dataset's baseline of n=100,000. Unlike bench_huge3.c (n-only, fixed
 * ttl_span=distinct=10000), this accepts all four parameters so the exact
 * same sweep design as the main suite (src/benchmarks/c/bench.c) can be
 * rerun with n itself huge, to check whether the crossover/overflow/
 * workload effects look the same at that scale.
 *
 *   ./bench_huge4 <op> <algo> <axis> <n> <ttl_span> <distinct> <workload>
 * appends one row to results_c/huge_full_sweep/<op>_<axis>.csv (axis is
 * just a label for the filename/column header, matching bench.c's
 * convention; it does not change what is measured, only the file it's
 * written to and the value written in the first CSV column).
 *
 * Same memory guard as bench_huge3.c (isolated subprocess, live vm_stat
 * check, currently 97% utilization), so it is safe to invoke many times in
 * a row without risking the host.
 */
#include "cts.h"
#include "util.h"
#include <stdio.h>
...
int main(int argc, char **argv) {
    if (argc != 8) {
        fprintf(stderr, "usage: %s <op> <algo> <axis_label> <n> <ttl_span> <distinct> <workload>\n", argv[0]);
        return 2;
    }
    ...
    if (!memory_ok(algoname, n, &need_gb, &budget_gb)) {
        printf("SKIP %-8s %-13s axis=%-13s n=%-11zu (needs ~%.1fGB, safe budget ~%.1fGB)\n",
               opname, algoname, axis, n, need_gb, budget_gb);
        return 0;
    }
    ...
    printf("OK   %-8s %-13s axis=%-13s n=%-11zu ttl_span=%-9llu distinct=%-7llu %-8s mean=%.2f  (%.1fs)\n",
           opname, algoname, axis, n, (unsigned long long)ttl_span, (unsigned long long)distinct, argv[7], agg.mean, secs);
    return 0;
}
